...
const port = Number(process.env.DATASWARM_E2B_TEMPLATE_VERIFICATION_E2E_PORT ?? 3229);
const baseUrl = `http://localhost:${port}`;
const dataDir = path.resolve(root, process.env.DATASWARM_DATA_DIR ?? "data");
const dbPath = path.join(dataDir, "dataswarm.sqlite");
const fakeApiKey = "e2b_fake_secret_for_template_verification_smoke";
const smokeTitle = "Smoke E2B template verification gate";
const results = [];
let server;
...
try {
  if (process.env.DATASWARM_E2B_TEMPLATE_VERIFICATION_E2E_SKIP_BUILD !== "1") {
  ...
  expect("snapshot reports api key configured without leaking it", readiness?.apiKeyConfigured === true && !JSON.stringify(readiness).includes(fakeApiKey), JSON.stringify(readiness));
  expect("snapshot gates orchestrator on template verification", readiness?.templateVerified === false && readiness?.status === "needs_template_verification" && readiness?.readyForLiveSmoke === true && readiness?.readyForOrchestrator === false, JSON.stringify(readiness));
  expect("snapshot missing env only reports template verification receipt", Array.isArray(readiness?.missingEnv) && !readiness.missingEnv.includes("E2B_API_KEY") && readiness.missingEnv.some((item) => String(item).includes("DATASWARM_E2B_TEMPLATE_VERIFIED")), JSON.stringify(readiness));
  ...
  const accepted = await postJson(`/api/conversations/${conversationId}/messages`, {
    text: "请用 swarm 多分支沙箱执行一次 E2B template verification gate 诊断验证",
    model: "dmx:claude-opus-4-8",
    mode: "agent",
  });
  const runId = accepted?.run_id;
  expect("template verification gate swarm message accepted", typeof runId === "string", JSON.stringify(accepted));
  ...
  try {
    const sandboxRows = db
      .prepare(
        `SELECT status, provider, external_sandbox_id, metadata_json
    ...
    expect("template verification gate creates branch sandbox sessions", sandboxRows.length === 3, `${sandboxRows.length} sandbox session(s)`);
    expect("all branch sessions use e2b provider", sandboxRows.every((row) => row.provider === "e2b"), JSON.stringify(sandboxRows));
    expect("all branch sessions fail before external sandbox creation", sandboxRows.every((row) => row.status === "failed" && row.external_sandbox_id === null), JSON.stringify(sandboxRows));
    expect(
      "sandbox metadata records template verification preflight evidence",
      metadata.every(
        (item) =>
          item.provider_mode === "e2b" &&
          item.error_code === "sandbox_preflight_failed" &&
          item.e2b_preflight?.status === "needs_template_verification" &&
          item.e2b_preflight?.api_key_configured === true &&
          ...
          !JSON.stringify(item).includes(fakeApiKey),
      ),
    ...
    expect(
      "branch failed events carry template verification missing env",
    ...
    expect(
      "template-gated e2b branches create failed observations",
    ...
    expect(
      "template-gated branch failed events link observations",
    ...
    expect(
      "template-gated branch observation.created events persisted",
    ...
    expect(
      "template-gated swarm merge carries branch observation ids",
    ...
    expect(
      "template-gated swarm verify records failed branch evidence",
...
async function waitForHealth(output) {
  ...
  while (Date.now() < deadline) {
    ...
    if (response?.ok) {
      expect("template verification gate server healthy", true, baseUrl);
      return;
    }
  ...
  expect("template verification gate server healthy", false, output.join("\n").slice(-3000));
  finish();
}
...
function finish() {
  ...
  if (failed.length > 0) {
    console.error(`\nE2B template verification e2e smoke failed: ${failed.length}/${results.length} check(s) failed.`);
    process.exit(1);
  }
  console.log(`\nE2B template verification e2e smoke passed: ${results.length}/${results.length} check(s) passed.`);
  process.exit(0);
}
