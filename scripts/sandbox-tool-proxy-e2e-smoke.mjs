...
try {
  ...
  const artifactCreateResponse = await postProxy({
    ...
    input: {
      ...
      title: "DataSwarm Proxy Artifact Evidence Report",
      content: [
        "# DataSwarm Proxy Artifact Evidence Report",
        "",
        "## Executive Summary",
        "",
        "This report is produced through the parent tool proxy to prove that sandbox agents can create durable Markdown deliverables through artifact.create. It intentionally contains enough narrative analysis to satisfy the same minimum substance checks used by swarm verification, rather than relying on a short runtime summary.",
        "",
        "## Evidence Coverage",
        "",
        "The artifact.create capability records a parent tool_call, persists an Observation, creates a Markdown artifact, and stores qualitySignals describing character count, section count, evidence citations, and deliverable eligibility. In live branch runs, sourceObservationIds should be supplied so the report can cite concrete Observation ids from web.search, file.read, trace.query, or run_python outputs.",
        "",
        "## Delivery Implications",
        "",
        "A substantive branch artifact must be useful to a downstream reducer. That means it needs a clear conclusion, evidence-oriented sections, limitations, and provenance. Thin reports or runtime log summaries should remain visible for diagnostics, but they must not count toward user-facing deliverable coverage in V4.1 verification.",
        "",
        "## Limitations",
        "",
        "This smoke artifact uses the proxy fixture rather than live E2B evidence, so it proves artifact.create substance classification and persistence mechanics only. Live complex benchmark acceptance still requires real branch observations and sourceObservationIds from the running swarm.",
        "",
      ].join("\n"),
    },
  });
