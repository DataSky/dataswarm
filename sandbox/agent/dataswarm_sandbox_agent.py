...
def build_action_system_prompt(job: Dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "Use final_answer only when the branch has enough observations or must stop due to budget.",
            "final_answer should include usedObservationIds, artifactIds, and limitations when available.",
            "Before final_answer, prefer read_context if scoped context is available and has not been observed yet.",
            "Before final_answer, prefer call_tool when the objective requires external/current facts and tool budget remains.",
            "Before final_answer, prefer run_python when the objective asks for a plot, image, code execution, or computed artifact and no artifact exists yet.",
